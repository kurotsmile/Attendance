...
var setting_show_timer='show';

$(document).ready(function(){
    $("#menu_info").hide();
    if(localStorage.getItem("pos_btn_add")!=null) position_btn_add=localStorage.getItem("pos_btn_add");
    if(localStorage.getItem("setting_show_timer")!=null) setting_show_timer=localStorage.getItem("setting_show_timer");
    load_list();
    load_pi();
});
...
function load_pi(){
    ...
    for(var pi=1;pi<=length_pi;pi++){
        ...
        if(setting_show_timer=="show") html_pi+="<span id='timer_"+pi+"' class='timer'>None ☢</span>";
        html_pi+="</div>";
    }
    html_pi+='</div>';
...
function show_setting(){
    ...
    html+='<div class="row">';
        ...
        html+='<div class="col-4">';
            html+='<select id="setting_show_timer" class="form-control">';
                html+='<option value="show" '+(setting_show_timer==='show' ? ' selected' : '') +'>Show</option>';
                html+='<option value="hide" '+(setting_show_timer==='hide' ? ' selected' : '') +'>Hide</option>';
            html+='</select>';
        html+='</div>';
    html+='</div>';
    ...
    html+='<div class="row">';
        ...
        html+='</div>';
        html+='<div class="col-4">';
            html+='<input class="form-control" id="length_pi" value="'+length_pi+'"/>';
        html+='</div>';
    html+='</div>';
    ...
    Swal.fire({html:html,showCancelButton: true}).then((result) => {
        if (result.isConfirmed) {
            ...
            setting_show_timer=$("#setting_show_timer").val();
            length_pi=parseInt($("#length_pi").val());

            localStorage.setItem('pos_btn_add',position_btn_add);
            localStorage.setItem("setting_show_timer",setting_show_timer);
            localStorage.setItem(id_table+"_lenth_app",length_pi);
